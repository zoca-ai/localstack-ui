...
export async function GET(
  ...
  try {
    ...
    const historyItemType = searchParams.get("historyItemType") as
      | "ConfigurationUpdate"
      | "StateUpdate"
      | "Action"
      | undefined;
    const startDate = searchParams.get("startDate")
      ? new Date(parseInt(searchParams.get("startDate")!))
      : undefined;
    const endDate = searchParams.get("endDate")
      ? new Date(parseInt(searchParams.get("endDate")!))
      : undefined;
    const maxRecords = searchParams.get("maxRecords")
      ? parseInt(searchParams.get("maxRecords")!)
      : 100;
    const nextToken = searchParams.get("nextToken") || undefined;
    const scanBy = searchParams.get("scanBy") as
      | "TimestampDescending"
      | "TimestampAscending"
      | undefined;
