...
export async function GET(request: NextRequest) {
  try {
    ...
    if (operation === "query") {
      ...
      const expressionAttributeNames = searchParams.get(
        "expressionAttributeNames",
      );
      const expressionAttributeValues = searchParams.get(
      ...
      if (expressionAttributeNames) {
        queryParams.ExpressionAttributeNames = JSON.parse(
          expressionAttributeNames,
        );
      }

      if (expressionAttributeValues) {
        queryParams.ExpressionAttributeValues = JSON.parse(
          expressionAttributeValues,
        );
      }
