...
export async function GET(
  ...
  try {
    ...
    const attachedPoliciesCmd = new ListAttachedUserPoliciesCommand({ UserName: userName });
    ...
    try {
      const loginProfileCmd = new GetLoginProfileCommand({ UserName: userName });
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to get IAM user' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to delete IAM user' },
      { status: 500 }
    );
  }
}