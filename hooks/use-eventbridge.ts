  ...
  return useMutation({
    mutationFn: async (name: string) => {
      const response = await fetch(
        `/api/eventbridge/buses?name=${encodeURIComponent(name)}`,
        {
          method: "DELETE",
        },
      );
...
export function useEventRule(
  ...
  return useQuery<EventRule>({
    ...
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/eventbridge/rules/${encodeURIComponent(name)}?${params}`,
      );
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["event-targets", variables.rule, variables.eventBusName],
      });
    },
  });
}
