...
export function FunctionViewer({ func, open, onOpenChange }: FunctionViewerProps) {
  const { data: functionDetails, isLoading, error } = useLambdaFunction(
    open && func ? func.functionName : null
  );
        ...
        <DialogHeader>
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <DialogTitle className="text-xl">{details.functionName}</DialogTitle>
        ...
        <Tabs defaultValue="configuration" className="mt-6">
              ...
              <div className="grid grid-cols-2 gap-4 rounded-lg border p-4">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm font-medium">{details.timeout || 3} seconds</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 rounded-lg border p-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-medium">{formatBytes(details.codeSize || 0)}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-muted-foreground">Last Modified</p>
                  <p className="text-sm font-medium">
                    {details.lastModified
                      ? formatDistanceToNow(new Date(details.lastModified)) + ' ago'
                      : 'Unknown'}
                  </p>
                </div>
          ...
          <TabsContent value="environment" className="mt-4 space-y-6">
            ...
            <div className="space-y-4">
              ...
              {details.environment?.variables && Object.keys(details.environment.variables).length > 0 ? (
                <ScrollArea className="h-[300px] rounded-lg border p-4">
                  <div className="space-y-2">
                    {Object.entries(details.environment.variables).map(([key, value]) => (
              ...
              {details.vpcConfig?.vpcId ? (
                <div className="rounded-lg border p-4 space-y-3">
                  <div>
                    ...
                    <p className="text-sm font-mono">{details.vpcConfig.vpcId}</p>
                  </div>
                  {details.vpcConfig.subnetIds && details.vpcConfig.subnetIds.length > 0 && (
                    <div>
                  ...
                  {details.vpcConfig.securityGroupIds && details.vpcConfig.securityGroupIds.length > 0 && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Security Groups</p>
                      <div className="flex flex-wrap gap-2">
                        {details.vpcConfig.securityGroupIds.map((sg) => (
                          <Badge key={sg} variant="secondary" className="font-mono text-xs">
                            {sg}
                          </Badge>
                        ))}
                      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg border p-4 space-y-2">
                ...
                <div>
                  ...
                  <p className="text-sm font-mono">{details.version || '$LATEST'}</p>
                </div>
                {details.state && (
                  <>
                    <Separator />
                    <div>
                      ...
                      <Badge variant={details.state === 'Active' ? 'default' : 'secondary'}>
                        {details.state}
                      </Badge>
                      {details.stateReason && (
                        <p className="text-xs text-muted-foreground mt-1">{details.stateReason}</p>
                      )}
                    </div>
                  </>
                )}
