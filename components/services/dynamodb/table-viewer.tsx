  ...
  if (error) {
    return (
      <div className="text-center py-8">
        <p className="text-destructive">Failed to load table details: {error.message}</p>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground">
            Created {formatDistanceToNow(new Date(table.creationDateTime), { addSuffix: true })}
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{table.itemCount.toLocaleString()}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{formatBytes(table.tableSizeBytes)}</div>
              </CardContent>
            </Card>

            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-medium">Billing Mode</CardTitle>
              </CardHeader>
              <CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {table.keySchema?.map((key: { attributeName: string; keyType: string }) => (
                  <div key={key.attributeName} className="flex items-center gap-2">
                    {key.keyType === 'HASH' ? (
                    ...
                    <Badge variant="secondary">
                      {table.attributeDefinitions?.find(
                        (attr: { attributeName: string; attributeType: string }) => attr.attributeName === key.attributeName
          ...
          {table.provisionedThroughput && table.billingMode !== 'PAY_PER_REQUEST' && (
            <Card>
              <CardHeader>
                <CardTitle>Provisioned Throughput</CardTitle>
                <CardDescription>Read and write capacity units</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-muted-foreground">Read Capacity</p>
                    <p className="text-2xl font-bold">
                      {table.provisionedThroughput.readCapacityUnits} RCU
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Write Capacity</p>
                    <p className="text-2xl font-bold">
        ...
        <TabsContent value="indexes" className="mt-4 space-y-4">
          {table.globalSecondaryIndexes && table.globalSecondaryIndexes.length > 0 ? (
            <>
              <h3 className="text-lg font-semibold">Global Secondary Indexes</h3>
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      <div>
                        ...
                        {gsi.keySchema?.map((key: { attributeName: string; keyType: string }) => (
                          <div key={key.attributeName} className="flex items-center gap-2 mt-1">
                            {key.keyType === 'HASH' ? (
                              <Hash className="h-3 w-3 text-muted-foreground" />
                            ) : (
                              <SortAsc className="h-3 w-3 text-muted-foreground" />
                            )}
                            <span className="text-sm">{key.attributeName}</span>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">No secondary indexes configured</p>
            </div>
          )}

          {table.localSecondaryIndexes && table.localSecondaryIndexes.length > 0 && (
            <>
              <h3 className="text-lg font-semibold mt-6">Local Secondary Indexes</h3>
              {table.localSecondaryIndexes.map((lsi: any) => (
                <Card key={lsi.indexName}>
                  <CardHeader>
                    <CardTitle className="text-base">{lsi.indexName}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      <div>
                        <p className="text-sm font-medium">Key Schema</p>
                        {lsi.keySchema?.map((key: { attributeName: string; keyType: string }) => (
                          <div key={key.attributeName} className="flex items-center gap-2 mt-1">
                            {key.keyType === 'HASH' ? (
                              <Hash className="h-3 w-3 text-muted-foreground" />
                            ) : (
